...
import { generatedCasesFromProcess, previousRelease, reqVersionImpact, sortReleases } from '@/utils/qaProcess'
...
const draftCaseCount = computed(() =>
  requirements.value.reduce((n, r) => n + ((r.draft_cases || []).length), 0),
)
...
const pageDesc = computed(() => {
  if (view.value === 'library') {
    const extra = draftCaseCount.value ? `含 ${draftCaseCount.value} 条草稿。` : ''
    return `模块按脑图路径写，用 - 分开。用例来自流程草稿。${extra}`
  }
  if (view.value === 'mindmap') return '脑图按端拆开测试点，叶子写成用例。'
  return '应用图谱相对版本。默认看最新版，可在右侧切换。'
})
...
const selectedReqImpact = computed(() => {
  if (!selectedReq.value) return null
  return reqVersionImpact(selectedReq.value, releases.value, versionAtlas.value)
})
...
const mindDiff = computed(() => summarizeMindDiff(mindRoot.value))
const mindGuideSummary = computed(() => {
  const bits = []
  if (activeRelease.value) bits.push(`当前 ${activeRelease.value.title}`)
  if (mindDiff.value.addedPoints.length) bits.push(`新增 ${mindDiff.value.addedPoints.length} 个测试点`)
  if (selectedReq.value) bits.push(selectedReq.value.title)
  return bits.join(' · ') || '按端拆开的覆盖清单'
})
const boardRoot = computed(() => (view.value === 'mindmap' ? mindRoot.value : atlasRoot.value))
const selectedReqImpactInfo = computed(() => selectedReq.value?.understanding?.impact || {})
const selectedReqPlatforms = computed(() => inferReqPlatforms(selectedReq.value))
const changeKindLabel = (kind) => ({ new: '全新能力', optimize: '优化已有功能', unknown: '材料不足，先推断' }[kind] || '')
  ...
  <div class="settings-panel cases-workbench wide-panel" v-loading="loading">
    ...
    <section v-if="view === 'library'" class="library-wrap is-table">
      <HintFold
        v-if="!hideNav"
        title="用例表"
        :summary="draftCaseCount ? `模块按脑图路径写。含 ${draftCaseCount} 条草稿。` : '模块按脑图路径写，用 - 分开。'"
      >
        <p>模块文案按脑图路径写，用 - 分开。支持筛选和翻页。<span v-if="draftCaseCount">含 {{ draftCaseCount }} 条草稿。</span></p>
      </HintFold>
    ...
    <section v-else class="mind-page">
      ...
      <HintFold
        ...
        :summary="mindGuideSummary"
      >
        <p>
          脑图按端拆开（App / Web / 端到端），端下面才是产品流程上的测试点，不是另一张产品结构。叶子写成用例；需求QA BM 用这些点看覆盖。
          {{ activeRelease ? `当前版本 ${activeRelease.title}` : '还没挂版本' }}。
          {{ prevRelease ? `对比上一版 ${prevRelease.title}` : '没有上一版本，下面标「新」的都是本版新增' }}。
        </p>
        <p v-if="selectedReqImpact">
          {{ selectedReqImpact.target ? `这条需求挂在 ${selectedReqImpact.target.title}` : '这条需求还没挂到版本' }}。
          {{ selectedReqImpact.label }}。
        </p>
        <p v-if="selectedReq?.understanding?.change_kind">
          {{ changeKindLabel(selectedReq.understanding.change_kind) }}。
          <span v-if="selectedReq.understanding.baseline">原来：{{ selectedReq.understanding.baseline }}</span>
          <span v-if="selectedReq.understanding.delta"> 这次：{{ selectedReq.understanding.delta }}</span>
        </p>
        <p v-if="selectedReqPlatforms.length || selectedReqImpactInfo.how_to_run">
          涉及 {{ selectedReqPlatforms.map(platformLabel).join('、') || '未标明端' }}。
          <span v-if="selectedReqImpactInfo.e2e">需要端到端。</span>
          <span v-if="selectedReqImpactInfo.how_to_run">怎么跑：{{ selectedReqImpactInfo.how_to_run }}</span>
        </p>
        <p v-if="mindDiff.addedPoints.length">
          新增 {{ mindDiff.addedPoints.length }} 个测试点
          <span v-if="mindDiff.addedFeatures.length">、{{ mindDiff.addedFeatures.length }} 条路径</span>
          ：{{ mindDiff.addedPoints.slice(0, 8).map((p) => p.name).join('、') }}{{ mindDiff.addedPoints.length > 8 ? '…' : '' }}
        </p>
      </HintFold>
      <section v-if="pendingPatches.length && view === 'atlas'" class="atlas-review">
        ...
        <p class="atlas-review-hint">确认骨架不等于单据评审通过。下面仍显示已确认的版本。</p>
        <AtlasChangeReview
      ...
      <section v-if="!(boardRoot.children || []).length" class="settings-info-card">
        <div class="settings-kicker">{{ view === 'mindmap' ? '还没有测试脑图' : '还没有已确认的应用图谱' }}</div>
        <p v-if="pendingPatches.length">先在本页确认图谱，确认后才会写入并重跑脑图和用例。</p>
        <p v-else>贴需求或点「继续分析」。图谱和脑图都支持列表 / 脑图两种看法。</p>
      </section>
    ...
    <el-dialog
      ...
      destroy-on-close
    >
      <p class="hint">写明为什么这份影响范围不对，以及你认为原来的功能和这次改动分别是什么。</p>
      <el-input
        v-model="rejectNote"
        type="textarea"
        :rows="5"
        placeholder="例如：这是已有工具链路的优化，不是新模块。"
      />
      <template #footer>
