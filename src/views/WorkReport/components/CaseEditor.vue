      ...
      if (graphId.value) {
        ...
        if (detailRes.code === 200) {
          ...
          nodes.value = rawNodes.map(n => ({
            ...
            data: {
              ...(n.data || {}),
              interactions: (n.data?.interactions || []).map(i => i.rect ? { ...i, x: i.rect.x, y: i.rect.y, w: i.rect.w, h: i.rect.h } : i),
          ...
          if (caseNodes.length > 0) {
            Promise.all(caseNodes.map(async (node) => {
              ...
              } catch (e) { console.error('Fetch workflow detail failed', e) }
            }))
          }
...
watch(() => route.fullPath, () => { loadGraphData() })
...
const handleKeydown = (e) => {
  ...
  switch (e.code) {
    case 'Space': e.preventDefault(); selectedNode.value = selectedElements.value[0]; break
    case 'Enter': e.preventDefault(); addSiblingNode(); break
    case 'Tab': e.preventDefault(); e.shiftKey ? addParentNode() : addChildNode(); break
  }
}
...
const onConnect = async (params) => {
  ...
  if (sourceNode && targetNode) {
    let parentId = null; let childNode = null
    const priority = { page: 3, component: 2, case: 1 }
    const sLevel = priority[sourceNode.type] || 0
    const tLevel = priority[targetNode.type] || 0
    if (sLevel >= tLevel) { parentId = sourceNode.id; childNode = targetNode }
    else { parentId = targetNode.id; childNode = sourceNode }
    ...
    if (childNode && parentId) {
      ...
      try { await api.saveNodeDetail(payload) } catch (e) { console.error('Save parentNode failed', e) }
    }
  }
  triggerAutoSave()
}

const onSelectionChange = (elements) => { selectedElements.value = elements.nodes || [] }
const onNodeClick = () => {}
...
const onNodeDoubleClick = ({ node }) => {
  if (node.type === 'case') {
    ...
    } else { router.push({ name: 'Editor', params: { id: node.id } }) }
    return
  }
...
const handleSaveLayout = async () => {
  try {
    if (!graphId.value) { if (!await ensureGraphId()) { saveStatus.value = 'unsaved'; return } }
    const saveNodes = nodes.value.map(n => ({ id: n.id, position: n.position, type: n.type, parentNode: n.parentNode, data: { ...n.data, label: n.label || n.data.label } }))
    const saveEdges = edges.value.map(e => ({ id: e.id, source: e.source, target: e.target, sourceHandle: e.sourceHandle, label: e.label, trigger: e.data?.trigger }))
    await api.syncGraphLayout({ graph_id: graphId.value, nodes: saveNodes, edges: saveEdges })
    saveStatus.value = 'saved'
  } catch (e) { saveStatus.value = 'unsaved' }
}
...
const onNodeUpdate = async (updatedNode) => {
  ...
  try { await api.saveNodeDetail(payload); triggerAutoSave() } catch (error) { ElMessage.error('保存失败') }
}
...
const createNodeData = (type, position, label) => {
  ...
  return {
    ...
    data: { label: label || labelMap[type], type, desc: '', ...(type === 'page' ? { naturalSize: { w: 375, h: 667 }, interactions: [] } : {}), ...(type === 'case' ? { workflow_id: null } : {}) }
  }
}
...
const initWorkflowIfCase = async (node) => {
  if (node.type === 'case' && !node.data.workflow_id) {
    try {
      const content = { nodes: [{ id: `public-trigger-${Date.now()}`, type: 'custom', position: { x: 100, y: 200 }, data: { label: '开始', nodeCode: 'public/trigger', outputs: [] } }], edges: [] }
    ...
    } catch (e) { ElMessage.warning('创建关联流程失败') }
  }
}
...
const addNode = async (type) => {
  ...
  try {
    ...
    if (gid) {
      await api.addEmptyNode({ graph_id: gid, node_id: newNode.id, type: newNode.type, label: newNode.label, x: parseInt(position.x), y: parseInt(position.y) })
      await api.saveNodeDetail({ graph_id: gid, node_id: newNode.id, type: newNode.type, label: newNode.label, desc: '', parentNode: null, naturalSize: newNode.data.naturalSize || null, screenshot: null, workflow_id: newNode.data.workflow_id ? String(newNode.data.workflow_id) : null, components: [] })
    }
    nodes.value.push(newNode)
  } catch (error) { ElMessage.error('添加失败'); return }

  if (parentNode) {
    setTimeout(() => { flowInstance?.addEdges([{ id: `e-${parentNode.id}-${newNode.id}`, source: parentNode.id, target: newNode.id, type: 'smoothstep' }]) }, 10)
    triggerAutoSave()
  }
}
...
const addChildNode = async () => {
  ...
  try {
    const gid = await ensureGraphId()
    if (gid) {
      await api.addEmptyNode({ graph_id: gid, node_id: newNode.id, type: newNode.type, label: newNode.label, x: parseInt(newNode.position.x), y: parseInt(newNode.position.y) })
      await api.saveNodeDetail({ graph_id: gid, node_id: newNode.id, type: newNode.type, label: newNode.label, desc: '', parentNode: null, naturalSize: newNode.data.naturalSize || null, screenshot: null, workflow_id: newNode.data.workflow_id ? String(newNode.data.workflow_id) : null, components: [] })
    }
    nodes.value.push(newNode)
  } catch (error) { return }
  setTimeout(() => { flowInstance?.addEdges([{ id: `e-${parent.id}-${newNode.id}`, source: parent.id, target: newNode.id, type: 'smoothstep' }]) }, 10)
  triggerAutoSave()
}
...
const addParentNode = async () => {
  ...
  setTimeout(() => { flowInstance?.addEdges([{ id: `e-${newNode.id}-${child.id}`, source: newNode.id, target: child.id, type: 'smoothstep' }]) }, 10)
  triggerAutoSave()
}
...
const removeSelected = () => {
  ...
  selectedElements.value = []; triggerAutoSave()
}
...
.editor-toolbar-glass {
  height: 100%; display: flex; align-items: center; gap: 20px; padding: 0 24px;
...
.label { font-size: 13px; color: #4b5563; font-weight: 700; }
.toolbar-info { margin-left: auto; display: flex; align-items: center; gap: 15px; }
...
.flow-canvas { background: transparent !important; }

/* 状态标签 */
.save-status { font-size: 12px; font-weight: 700; padding: 4px 12px; border-radius: 20px; }
.save-status.saving { background: rgba(230, 162, 60, 0.1); color: #e6a23c; }
.save-status.saved { background: rgba(103, 194, 58, 0.1); color: #67c23a; }
.save-status.unsaved { background: rgba(245, 108, 108, 0.1); color: #f56c6c; }
