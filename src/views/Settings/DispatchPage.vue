...
<template>
  <div class="settings-panel dispatch-page wide-panel" v-loading="loading">
    ...
    <section v-if="!rows.length && !loading" class="settings-info-card">
      <div class="settings-kicker">还没有调用记录</div>
      <p>{{ appId ? '在这个应用里推进单据、确认图谱或跑用例后，这里会出现角色调用。' : '推进单据、确认图谱、跑用例，或在角色页对话，这里会出现记录。' }}</p>
    </section>
