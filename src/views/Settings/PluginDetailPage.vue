...
<template>
  <div class="settings-panel plugin-detail wide-panel" v-loading="loading">
    ...
    <section v-if="!loading && !plugin" class="settings-info-card">
      <div class="settings-kicker">没有这个插件</div>
      <p>从全部插件里重新选一个。</p>
    </section>
    ...
    <template v-else-if="isIm && tab === 'chat'">
      <div class="im-chat-page">
        ...
        <HintFold
          ...
          title="飞书收不到消息时"
          :summary="chatListener.connected ? '长连接已通，一般不用看' : '要长连接和企业自建应用，不要填本机地址'"
          :default-open="setupFoldOpen"
        >
          <p>开放平台 → 事件与回调 → 用<strong>长连接</strong>收事件（先开着 MiniOrange 再保存）。</p>
          <p>订阅 <code>im.message.receive_v1</code>，权限加 <code>im:message</code>，然后发布应用。</p>
          <p>私聊的必须是「连接」里那个企业自建应用。只有 webhook 的自定义机器人不能对话。</p>
          <p>备用 HTTP：<code>http://公网地址:10104{{ chatWebhookPath }}</code>，本机直填无效。</p>
        </HintFold>
        ...
        <section class="settings-card im-trial">
          ...
          <div class="im-trial-log">
            <div v-if="!chatTrial.messages.length" class="im-trial-empty">
              先说一句，例如「登录失败了下一步怎么查」
            </div>
            <div
    ...
    <section v-else-if="pluginId === 'feishu' && tab === 'notify'" class="settings-card">
      ...
      <p class="hint">跑失败、图谱待确认、验收/发版结论可以推到飞书群。用上面「连接」里的机器人发。</p>
      <el-form label-position="top" class="settings-form-stack">
    ...
    <section v-else-if="pluginId === 'figma' && tab === 'connect'" class="settings-card">
      ...
      <p class="hint">
        在 figma.com → 头像 → Settings → Security 生成 Personal access token，勾选 file_content:read。不需要 Developer OAuth。
      </p>
      <el-form label-position="top" class="settings-form-stack">
        <el-form-item :label="plugin?.figma?.configured ? 'Token（已配置，输入新值可覆盖）' : 'Token'">
          <el-input
    ...
    <section v-else-if="pluginId === 'figma' && tab === 'bind'" class="settings-table-card">
      <div class="card-head">
        <div>
          <h3>应用设计稿</h3>
          <p>Token 全局一份；每个应用自己绑文件。同步和学习仍在应用配置里执行。</p>
        </div>
      </div>
      <el-table :data="bindings" empty-text="还没有应用">
    ...
    <section v-else-if="pluginId === 'zentao' && tab === 'connect'" class="settings-card">
      ...
      <p class="hint">用账号密码向禅道开放接口换 Token。密码只用来换票，不会保存。也可以自己粘贴个人访问令牌。MiniOrange 缺陷单是源，禅道是公司流程里的副本。</p>
      <el-form label-position="top" class="settings-form-stack">
        ...
        <el-form-item label="密码">
          <el-input v-model="zentaoForm.password" type="password" show-password placeholder="登录密码，仅用于换 Token" />
        </el-form-item>
        <div class="row-actions">
        ...
        <el-form-item :label="plugin?.config?.has_token ? 'Token（已配置，输入新值可覆盖）' : 'Token'">
          <el-input v-model="zentaoForm.token" type="password" show-password placeholder="个人访问令牌，可手填" />
        </el-form-item>
        <div class="row-actions">
    ...
    <section v-else-if="pluginId === 'zentao' && tab === 'bind'" class="settings-table-card">
      <div class="card-head">
        <div>
          <h3>项目 ↔ 禅道产品</h3>
          <p>一个 MiniOrange 项目对应一个禅道产品。缺陷从这里推过去。</p>
        </div>
      ...
      <el-table :data="zentaoBindings" empty-text="还没有绑定项目">
    ...
    <section v-else-if="pluginId === 'zentao' && tab === 'flow'" class="settings-card">
      ...
      <p class="hint">自动化失败先在 MiniOrange 建缺陷。推禅道默认要人点一次，避免刷爆。</p>
      <el-form label-position="top" class="settings-form-stack">
    ...
    <template v-else-if="pluginId === 'zentao' && tab === 'templates'">
      <section class="settings-table-card">
        ...
        <el-table :data="zentaoTemplates" empty-text="还没有模板">
      ...
      <section class="settings-card">
        ...
        <p class="hint">选一套模板和产品，往禅道丢一张真单。测完请关掉。</p>
        <el-form label-position="top" class="settings-form-stack">
